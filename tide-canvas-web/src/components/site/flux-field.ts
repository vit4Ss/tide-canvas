/* ============================================================================
   FluxField — "流光场" WebGL fragment-shader backdrop（domain-warped fbm flow）.
   design-ref/liuguang/flux-field.js 的 TS 移植：着色器与渲染循环逐行保留，
   IIFE/window.FluxField 改为 ES module 导出，并补了 destroy()（React 卸载时
   释放 rAF/监听器/GL 上下文——原稿页面级脚本从不卸载，没有这条路径）。

   mountFluxField(canvas, opts) — opts:
   ...
   - Falls back to a CSS gradient (.flux-fallback) if WebGL is unavailable.
   ========================================================================== */

export interface FluxMood {
  hue?: number;
  speed?: number;
  scale?: number;
  intensity?: number;
...
export interface FluxFieldOptions extends FluxMood {
  variant?: number;
  mouse?: boolean;
  res?: number;
}

export interface FluxFieldHandle {
  setMood(m: FluxMood): void;
  destroy(): void;
...
export function mountFluxField(
  ...
  let mtgt: [number, number] = [0.5, 0.55];
  const cur2: [number, number] = [0.5, 0.55];
  let energy = 0,
    tgtE = 0;
