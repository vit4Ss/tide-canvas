...
const sliderBounds = {
  ...
  armRFwd: [-90, 90], armRAbd: [-90, 45], armRTwist: [-80, 80],
...
test("procedural pose attributes match real sliders and stay within their limits", () => {
  for (const [pose, params] of Object.entries(POSE_PARAM_PRESETS)) {
    for (const [key, value] of Object.entries(params)) {
      const bounds = sliderBounds[key];
      assert.ok(bounds, `${pose} uses unknown pose attribute ${key}`);
      assert.ok(value >= bounds[0] && value <= bounds[1], `${pose}.${key} exceeds slider limits`);
    }
  }
});

test("the native XBot clips back standing, walking and running", () => {
  assert.deepEqual(Object.keys(SKINNED_ANIMATION_POSES), ["站立", "行走", "跑步"]);
});
