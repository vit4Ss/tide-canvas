...
/* ============================================================================
   ClipPicker — 延长/翻唱的原曲选择弹窗(画布音频节点与对话页共用)。

   替代原生下拉:Suno 一次生成两首同名歌,下拉框里全是重复标题无从分辨。
   这里按"封面 + 歌名 + 第 N 首标记 + 模型/时长/日期"逐行展示,并支持逐条
   试听,点行即选中。原曲只能来自本站生成历史(上游按 clip_id 归属校验,
   不支持上传本地音频)。
   ========================================================================== */

import { useEffect, useRef, useState } from "react";
import { createPortal } from "react-dom";
import { Check, Loader2, Music2, Pause, Play, X } from "lucide-react";
...
export function ClipPicker({ open, options, current, onClose, onPick }: Props) {
