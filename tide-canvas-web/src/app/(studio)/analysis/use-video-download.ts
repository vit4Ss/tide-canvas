...
const empty: DownloadState = { resultId: "", phase: "idle", loaded: 0, total: 0, name: "", error: "", savedUrl: "" };
...
export function useVideoDownload(ownerUserId: string) {
  ...
  const start = async (result: VideoDownloadResolveVO, maxBytes: number, onStarted: () => void, onFinished: () => void) => {
    ...
    if (saved.current?.id === result.id) {
      ...
      setState({ ...empty, resultId: result.id, owner: ownerUserId, name: file.name, phase: "ready", loaded: file.size, total: file.size, savedUrl: file.url });
      try { saveFile(file.url, file.name); } catch { /* The visible save link remains available. */ }
      return;
    }
    if (result.expiresAt * 1000 <= Date.now()) {
      setState({ ...empty, resultId: result.id, owner: ownerUserId, phase: "failed", name: result.fileName, error: "下载地址已过期，请重新获取视频" });
      return;
    }
    if (saved.current) URL.revokeObjectURL(saved.current.url);
    saved.current = null;
    ...
    setState({ ...empty, resultId: result.id, owner: ownerUserId, name, phase: "preparing" });
    onStarted();
    try {
      const blob = await receiveVideoDownload(apiUrl(result.downloadUrl), maxBytes, controller.signal, (progress) => {
        if (active.current === controller) setState({ ...empty, ...progress, resultId: result.id, owner: ownerUserId, name, phase: "receiving" });
      });
      if (active.current !== controller) return;
      const url = URL.createObjectURL(blob);
      saved.current = { id: result.id, url, name, size: blob.size };
      setState({ ...empty, resultId: result.id, owner: ownerUserId, name, phase: "ready", loaded: blob.size, total: blob.size, savedUrl: url });
      try { saveFile(url, name); } catch { /* The visible save link remains available. */ }
    } catch (error) {
      if (active.current !== controller) return;
      setState({ ...empty, resultId: result.id, owner: ownerUserId, name, phase: controller.signal.aborted ? "cancelled" : "failed", error: error instanceof Error ? error.message : "下载失败，请重试" });
    } finally {
