...
const channelNames = { feishu: "飞书", dingtalk: "钉钉", wecom: "企业微信", telegram: "Telegram" } as const;
  ...
  return <div className="alert-master-detail">
    ...
    <section className="alert-channel-editor">
      {creating || selected ? <>
        ...
        <div className="alert-form-section"><h3>基本信息</h3><div className="alert-form-grid"><label><span>渠道名称</span><input value={form.name} placeholder="如：运维告警群" onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}/></label><label><span>渠道类型</span><select value={form.type} disabled={!creating} onChange={(e) => setForm({ ...blankChannel(), name: form.name, enabled: form.enabled, type: e.target.value as AlertChannelInput['type'] })}><option value="feishu">飞书机器人</option><option value="dingtalk">钉钉机器人</option><option value="wecom">企业微信群机器人</option><option value="telegram">Telegram Bot</option></select></label></div></div>
        <div className="alert-form-section"><h3>连接配置</h3>{form.type !== "telegram" ? <div className="alert-form-grid single"><label><span>Webhook 地址</span><input type="password" autoComplete="new-password" value={form.config.webhook ?? ""} placeholder={form.type === "wecom" ? "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=…" : "https://…"} onChange={(e) => setForm((f) => ({ ...f, config: { ...f.config, webhook: e.target.value } }))}/><small>{form.type === "wecom" ? "粘贴企业微信群机器人的完整 Webhook；仅允许企业微信官方 HTTPS 域名，保存后加密存储。" : "仅允许该平台官方 HTTPS 域名，保存后加密存储。"}</small></label>{form.type !== "wecom" ? <label><span>签名密钥 <em>可选</em></span><input type="password" autoComplete="new-password" value={form.config.secret ?? ""} placeholder="未启用签名可留空" onChange={(e) => setForm((f) => ({ ...f, config: { ...f.config, secret: e.target.value } }))}/></label> : null}</div> : <div className="alert-form-grid"><label><span>Bot Token</span><input type="password" autoComplete="new-password" value={form.config.botToken ?? ""} onChange={(e) => setForm((f) => ({ ...f, config: { ...f.config, botToken: e.target.value } }))}/></label><label><span>Chat ID</span><input type="password" autoComplete="new-password" value={form.config.chatId ?? ""} onChange={(e) => setForm((f) => ({ ...f, config: { ...f.config, chatId: e.target.value } }))}/></label><label><span>Topic ID <em>可选</em></span><input value={form.config.threadId ?? ""} onChange={(e) => setForm((f) => ({ ...f, config: { ...f.config, threadId: e.target.value } }))}/></label></div>}</div>
        ...
        <div className="alert-editor-foot"><div>{!creating ? <button className="adm-btn ghost danger-text" onClick={remove}><Trash2 size={14}/>删除</button> : null}</div><div><button className="adm-btn ghost" disabled={creating || testing} onClick={test}><Send size={14}/>{testing ? "发送中…" : "发送测试"}</button><button className="adm-btn" disabled={saving} onClick={save}><Check size={14}/>{saving ? "保存中…" : "保存设置"}</button></div></div>
      </> : <AdminEmptyState title="尚未配置通知渠道" description="添加飞书、钉钉、企业微信或 Telegram，系统会向所有启用且命中规则的渠道发送通知。" action={<button className="adm-btn" onClick={create}><Plus size={15}/>添加渠道</button>}/>}
    </section>
  </div>;
}
