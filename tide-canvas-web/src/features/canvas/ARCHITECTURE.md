...
受控模式只允许单向数据流：`useCanvasStore → adapter → nodes/edges`。React Flow 发出的
`onNodesChange` 与 `onEdgesChange` 分别映射节点位置/选择和连线选择；`onSelectionChange`
仅是结果通知，不得作为第二个状态写入口。节点尺寸由框架内部测量，增删和连线端点变更必须走
明确的领域命令；没有对应命令的框架能力必须显式关闭，不能留下操作后回弹的假交互。
...
画布历史不从当前节点集合反推。服务端 `media_assets` 是生成结果与上传事件的交互索引，原始
`ai_tasks`、`files` 继续分别承担计费审计和存储配额职责。每个多结果输出都有稳定 ID；删除墓碑保证
旧数据回填、服务重启或终态重放都不会恢复已永久删除的输出。

前端边界如下：

- `canvas-history-panel` 只编排范围、媒体类型、游标分页、轮询和批量命令；
- `history-asset-card` 与 `history-asset-preview` 只负责输入方式无关的呈现和动作分发；
- `history-node-placement` 把资源转换为领域节点，在离视口中心最近的空白区域排成网格；
- “使用”通过一次 `addNodesAndConnections` 形成单个撤销步骤，不自动选中新节点，并立即请求持久化；
- 缩略图使用懒加载，预览才请求原始比例媒体；进行中任务只轮询当前可见的来源。
