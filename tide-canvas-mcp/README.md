...
列表和详情提供「复制安装指令」：用户粘贴给 Codex 等智能体后，由智能体下载并校验 SKILL.md、安装到本地 Skill 目录、记录公开来源并配置本技能的 MCP。安装与执行分开判定：技能已对外开放、已上架且格式有效即可获取安装包；MCP 总开关关闭、地址暂未配置或用户未设置 API Key，不阻止安装文件。接口通过 `mcpAvailable` / `mcpUnavailableReason` 单独说明连接条件；智能体缺少连接条件时报告「Skill 已安装，MCP 待连接」，不会为验证安装自动生成或扣费。缺少地址时不生成空 URL 配置，使用前从保存的来源读取技能详情中的最新地址。
...
复制的安装指令与公开 SKILL.md 都包含 MCP 自动接入流程：首次安装或后来发现连接缺失时，智能体先检查专属配置，再自动添加远程 HTTP MCP；已存在的连接复用，仅更新本技能过时的地址与鉴权。Codex 优先使用 `codex mcp get` / `codex mcp add`，没有 CLI 时在权限允许范围内合并用户级 config.toml。配置后尝试重连，只用 get_skill_info 验证；不支持热加载的客户端会明确提示重新连接或开启新会话。不会因本地未接入而要求用户另行部署 FlowLight 服务端。

登录用户点击「复制安装指令」或「复制 MCP 配置」时，前端通过本人鉴权的默认 Key 接口按需读取密钥，并加入剪贴板中的私人配置；使用账号 ID 和 Key 版本核对响应，切换账号或取消请求后不再复制。密钥不进入列表、预览、公开 SKILL.md、分享链接或 ZIP，也不在前端缓存。未登录时仍复制不含密钥的通用说明。此操作不会重置或启用已停用的 Key。
