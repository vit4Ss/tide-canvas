...
## 当前 UI 方向（用户 2026-09-05 明确指定，优先于下文旧约束）

用户要求：**“我的 UI 都符合科技感”“都应该让人眼前一亮”**。

- 全站后续 UI 新增和改版以科技感、视觉辨识度和第一眼吸引力为设计目标，同时保持可读性和操作效率。
- 历史记录就是历史记录：使用清晰的功能名称，不再改名为“记录簿”，不采用纸页、折角、宋体或杂志拟物风格。
- 延续项目深色主题，允许冷青强调色覆盖选中区域、几何边角、少量背景网格、层次渐变和适度光效；不再限于毫米级圆点。
- 下文“设计目标不是漂亮/炫酷/抢眼”“不要让用户注意设计”以及一概禁止科技风、发光和渐变的旧条款，在与本节冲突时不再适用。
- 视觉装饰应围绕真实内容与交互，不展示虚构的在线状态、扫描过程、进度、统计或技术参数。
- 保留真实状态、清晰错误提示、键盘焦点、触屏点击区域和减少动态效果支持。不得用常驻循环动画、大面积模糊或粒子效果造成卡顿。
- 保持已有功能及数据含义。全站统一方向通过后续具体 UI 任务逐步落实，不以此为由一次性改动未涉及的业务页面。
