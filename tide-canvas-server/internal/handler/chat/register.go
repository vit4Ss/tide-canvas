...
//	GET    /api/im/conversations/:id/stream            -> SSE 附着生成中的回复（断开重连续播）           (auth)
...
func Register(api *gin.RouterGroup, d *app.Deps) {
	svc := newService(d.DB, d.Cfg, d.Storage)
	h := newHandler(svc)
	...
	conv.PUT("/:id", h.renameConversation)
	conv.DELETE("/:id", h.removeConversation)
	conv.GET("/:id/messages", h.listMessages)
	conv.POST("/:id/messages", h.sendMessage)
	conv.POST("/:id/stream", h.streamMessage)
	conv.GET("/:id/stream", h.streamLiveMessage)
