...
// seedSkill is a baseline 技能广场 entry (official).
type seedSkill struct {
	title, description, category, outputType, promptTemplate, defaultParams string
}
...
// title + 官方 author, 与 ensureBaselineStyles 同款). Existing rows are never
// overwritten, so admin edits win over the seed. 注意:软删的种子行下次启动会
// 被重建(FirstOrCreate 查不到已删行)——要永久移除官方技能请「下架」而非删除。
func ensureBaselineSkills(db *gorm.DB) error {
	for i := range baselineSkills {
		s := baselineSkills[i]
		var row model.Skill
		res := db.Where(&model.Skill{Title: s.title, AuthorName: "官方"}).
			Attrs(&model.Skill{
