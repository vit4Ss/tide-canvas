...
func TestResolveCostVideoFuzzyMatrix(t *testing.T) {
	...
	for _, c := range cases {
		if got := resolveCost(m, json.RawMessage(c.input), 1); got != c.want {
...
func TestResolveCostImageFuzzyMatrix(t *testing.T) {
	...
	if got := resolveCost(m, json.RawMessage(`{"quality":"high","clarity":"2K"}`), 1); got != 30 {
		t.Errorf("uppercase clarity should hit lowercase matrix key: got %d, want 30", got)
	}
	// upscale 走 high/4k + 批量 1；未命中档位落固定价
	if got := resolveCost(m, json.RawMessage(`{"quality":"high","clarity":"8k"}`), 1); got != 18 {
