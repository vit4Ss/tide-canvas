...
// chatpool.go resolves a gateway model to the addresses that can serve it.
// The main site is the relay here: it holds the operator's third-party
// credentials and speaks the OpenAI protocol to them directly. A client only
// ever holds its own account key and this site's address, so a call cannot
// skip the per-user billing, and the operator's credentials never leave here.

var errNoChatModel = errors.New("chatgateway: model is not offered")
...
// mistakes one for the other may pick the wrong endpoint or the wrong feature
// set: several clients read a bare "gpt-5.x" id as an OpenAI model and switch
// behaviour on the id alone, which no provider setting overrides. Namespacing
// says whose model this is, the way "codex/" and "openai/" already do for
// other gateways. A client may still send the bare key; both resolve.
...
// enabled provider, with usable token pricing. A model missing any of the three is left
// out rather than shown and then refused at send time.
func (s *service) offeredModels(ctx context.Context) ([]chatRoute, error) {
	return offeredRoutes(ctx, s.d.DB)
}

// offeredRoutes is offeredModels on a bare DB handle, so surfaces outside the
// gateway (ListCatalogue) read the same rule.
func offeredRoutes(ctx context.Context, db *gorm.DB) ([]chatRoute, error) {
	var models []model.ChatModel
	if err := db.WithContext(ctx).Where("enabled = ?", true).Order(model.ChatModelOrder).Find(&models).Error; err != nil {
		return nil, err
	}
	if len(models) == 0 {
		return nil, nil
...
// displayNameOnly is the model's own name, for the model list.
