...
func TestTokenBillingLedgerIsReachableByThePointsModuleRole(t *testing.T) {
	...
	w := f.request("GET", "/api/admin/chat-gateway-billing", "", access, nil)
	if w.Code != 200 {
	...
	w = f.request("GET", "/api/admin/chat-gateway-billing?userId="+f.user.ID.String(), "", access, nil)
	...
	if w := f.request("GET", "/api/admin/chat-gateway-billing?userId=not-an-id", "", access, nil); w.Code == 200 {
	...
	if w := f.request("GET", "/api/admin/chat-gateway-billing", "", stranger, nil); w.Code == 200 {
...
func TestUserBillingListCannotReadAnotherAccount(t *testing.T) {
	...
	w := f.request("GET", "/api/chat-gateway/billing?userId="+other.ID.String(), "", f.jwt, nil)
	if w.Code != 200 {
