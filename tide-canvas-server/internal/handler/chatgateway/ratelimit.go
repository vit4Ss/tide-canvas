...
// Scope the gateway rate limit to the authenticated account rather than the
// caller's IP - a team's agents may all leave from one address - and return an
// OpenAI error with HTTP 429 rather than the main UI's HTTP-200 business envelope.
func (s *service) gatewayRateLimit() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		key := "ratelimit:chatgateway:" + middleware.CurrentUserID(c).String() + ":" + c.FullPath()
