...
// maxGatewayRequest bounds one chat request. Clients inline attached images as
// base64 — a few MiB each after their own compression — and an agent's history
// window keeps earlier messages that may carry images too. 16 MiB failed on a
// handful of pictures.
...
// billingKey is where a call leaves its billing summary on the gin context, for
// the error writers to attach.
const billingKey = "gateway.billing"
...
// OpenAI-compatible clients recognize insufficient_quota and show it as a
// balance problem — Codex classifies it as a quota failure and stops retrying.
// A custom HTTP 402 / insufficient_points would be displayed as an opaque
// server error.
...
func gatewayGenerationError(c *gin.Context, status int, code, message, modelName string, out *completion) {
	...
	if billing, ok := c.Get(billingKey); ok {
...
// reserve holds the worst-case cost of one call. Every gateway model is token
...
// decodeBody reads the single JSON object a call is. Too large and malformed
// are different problems for the person on the other end: one is fixed by
// attaching fewer pictures, the other is a client bug. Say which.
func (s *service) decodeBody(c *gin.Context) (map[string]any, bool) {
...
// call is one chat request after it has been resolved to a provider and its
// worst-case cost reserved. Both wire formats — Chat Completions and Responses
// — produce one of these; from here on the provider call and the billing do
// not know which dialect the client spoke.
type call struct {
	uid        idgen.ID
	modelName  string // the model as the client named it
	route      *chatRoute
	payload    []byte // the chat request the provider receives
	requestKey string
	row        *model.ModelGatewayRequest
	fresh      bool // false when row answers an idempotent replay
	stream     bool
	path       string
	started    time.Time
}
...
func replayInProgress(c *gin.Context, row *model.ModelGatewayRequest) bool {
...
// replayMessage says what became of a stored call that did not succeed.
func replayMessage(row *model.ModelGatewayRequest) string {
...
// outcome is how one provider conversation ended, settled and audited.
type outcome struct {
	out  completion
	code string // "" on success
	// terminal holds the frames from the first finish marker on. They were held
	// back from the client until the result was settled, so a client cannot
	// stop reading before the actual error arrives.
	terminal        string
	upstreamStatus  int
	upstreamMessage string
	message         string // what to tell the user when code != ""
}

// failureStatus is the HTTP status a non-streaming client gets for a failed
// call: the provider's own when it objected, 502 otherwise.
func failureStatus(r *outcome) int {
	if r.code == "upstream_error" && r.upstreamStatus >= 400 && r.upstreamStatus <= 599 {
		return r.upstreamStatus
	}
	return 502
}

// run makes the provider call for a fresh reservation, streaming every frame
// before the finish marker to live as it arrives, and settles the row. It
// returns once the ledger reflects the result; the caller says the last words
// to the client in its own dialect.
func (s *service) run(c *gin.Context, call *call, live func(wire string), heartbeat func()) *outcome {
	...
	for !ended {
		...
		case frame, ok := <-events:
			...
			if out.failed {
				...
				r.code = "upstream_error"
				r.upstreamMessage = out.errorMessage
				ended = true
				cancel()
			}
			if frame.data == "[DONE]" {
	...
	if r.code == "" && !out.complete() {
		r.code = "incomplete_response"
	}
	if r.code == "" {
	...
	if len(out.tools) > 0 {
		raw, _ := json.Marshal(out.json(call.modelName))
	...
	eventlog.ModelText(call.uid, "chat", call.route.model.ModelKey, call.path, eventlog.SanitizeDataURIs(string(call.payload)), auditResponse, call.started, callErr, cost, eventlog.ModelTextBillingRef{ID: row.ID, Type: "ledger"})
	if r.code != "" {
		r.message = "回复未完整生成，积分已退回，请重新发起或继续提问"
		if out.hasOutput() {
			r.message = fmt.Sprintf("回复未完整生成，已保留有效内容，本次消耗 %d 积分；继续提问会发起新调用", row.Cost)
		}
		if row.BillingMode == "token" {
			r.message = fmt.Sprintf("回复未完整生成，已按返回用量结算 %s 积分，未使用的预留额度已释放", tokenCostLabel(row.CostMicros))
			if row.Status == "billing_pending" {
				r.message = "本次 Token 用量需要核对，预留积分暂未释放，请查看主站 Token 账单"
			}
		}
		if r.code == "settlement_pending" {
			r.message = "结果结算暂未完成，请保留请求编号并稍后查询"
		}
		if r.code == "upstream_error" && r.upstreamMessage != "" {
...
// chat serves POST /chat/completions: the provider's own protocol, passed
// through with the credential swapped and the usage billed.
func (s *service) chat(c *gin.Context) {
	body, ok := s.decodeBody(c)
	if !ok {
		return
	}
	stream, _ := body["stream"].(bool)
	call := s.prepare(c, body, stream)
	if call == nil {
		return
	}
	row := call.row
	if !call.fresh {
		if replayInProgress(c, row) {
			return
		}
		if row.Status != "success" {
			...
			gatewayGenerationError(c, 502, "generation_failed", message, call.modelName, parseCompletion(row.ResponseBody))
			return
		}
		if stream {
			c.Header("Content-Type", "text/event-stream")
		...
		result := out.json(call.modelName)
		if row.BillingMode == "token" {
	...
	r := s.run(c, call, emit, func() { emit(": heartbeat\n\n") })
	out := &r.out
	if r.code != "" {
		if stream {
			emit(partialGatewayFrames(r.terminal))
			emit(billingFrame(row))
			detail := gin.H{"code": r.code, "type": r.code, "message": r.message}
			if r.upstreamStatus != 0 {
				detail["upstream_status"] = r.upstreamStatus
			}
			data, _ := json.Marshal(gin.H{"error": detail})
			emit("data: " + string(data) + "\n\ndata: [DONE]\n\n")
		} else {
			gatewayGenerationError(c, failureStatus(r), r.code, r.message, call.modelName, out)
		}
		return
	}
	if stream {
		emit(r.terminal + billingFrame(row) + "data: [DONE]\n\n")
	} else {
		result := out.json(call.modelName)
		if row.BillingMode == "token" {
			result["billing"] = billingInfo(row)
