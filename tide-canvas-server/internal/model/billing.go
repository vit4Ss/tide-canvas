...
// PointRefundReceipt is the durable idempotency claim for a refund. RefID is
// globally generated by the charging call and is therefore the natural primary
// key: a consume may be refunded at most once, regardless of how many workers
// retry the failure path. The receipt is committed in the same transaction as
// the balance credit and PointRecord row.
//
// This deliberately lives beside PointRecord instead of adding a unique index
// to the legacy ledger. Old databases may already contain duplicate refund
// rows, which would make AutoMigrate fail while creating such an index.
type PointRefundReceipt struct {
	RefID idgen.ID `gorm:"column:ref_id;primaryKey;autoIncrement:false" json:"refId"`
	// ClaimID identifies the transaction that created a receipt. It avoids
	// relying on MySQL's connection-dependent RowsAffected semantics for an
	// ON DUPLICATE KEY no-op. Legacy backfilled receipts keep the zero default.
	ClaimID    idgen.ID  `gorm:"column:claim_id;not null;default:0" json:"-"`
	UserID     idgen.ID  `gorm:"column:user_id;index;not null" json:"userId"`
	Amount     int       `gorm:"column:amount;type:int;not null" json:"amount"`
	CreateTime time.Time `gorm:"column:create_time;autoCreateTime" json:"createTime"`
}

func (PointRefundReceipt) TableName() string { return "point_refund_receipt" }
