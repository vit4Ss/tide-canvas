...
| 项目 | 实际规则 |
|---|---|
| 计费方式 | 按上游实际返回的 Token 用量结算：非缓存输入 × 输入单价 ÷ 1,000,000 ＋ 缓存输入 × 缓存单价 ÷ 1,000,000 ＋ 输出 × 输出单价 ÷ 1,000,000，向上取整到 0.000001 积分 |
| 单价配置 | 后台「模型管理 → 文本模型 → AI 聊天 · Token 定价」按每百万 Token 分别设置输入、输出、缓存输入积分，并配置单次输入/输出 Token 上限 |
| 未配置单价 | `TIDECANVAS_LOBEHUB_REQUIRETOKENPRICING=true`（部署脚本默认开启）时，未配置 Token 单价的模型不出现在 LobeHub 模型列表，调用也直接拒绝，不回退按次价格 |
