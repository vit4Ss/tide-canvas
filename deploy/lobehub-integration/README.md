...
| 凭证安全 | 第三方密钥加密存库、后台只显示是否已设置、从不下发前端；上游若在报错里回显密钥，网关只把密钥替换为 `[REDACTED]`，其余原文照返。接入地址允许 http（不少中转站只有 http），但明文传输意味着密钥在网络上裸露，后台会在该地址旁标出。内网 IP（10/8、172.16/12、192.168/16）可以直接填——中转站架在自己局域网里很常见；拒绝的是本机（127.0.0.1、0.0.0.0）和链路本地 169.254/16（云厂商的实例元数据服务，拿得到实例凭证），无论写成字面 IP 还是解析到这些地址的域名，拉取模型和聊天两条路径同一套判定。不跟随上游的 3xx 跳转（密钥不能跟着走），但会把跳转目标写进提示，通常是 http 该改成 https |
| 计费方式 | 由**模型自身配置**决定：打开 Token 计费的模型按上游实际返回的 Token 用量计算，非缓存输入 × 输入单价 ÷ 1,000,000 ＋ 缓存输入 × 缓存单价 ÷ 1,000,000 ＋ 输出 × 输出单价 ÷ 1,000,000；单次费用非零时最终向上取整到整数积分，计算结果为 0 时不扣费，结算结果写入统一积分明细 |
...
| 同一模型多家供应商 | 同一 `model_key` 出现在多个供应商下时，后台在该模型旁显示「首选 / 设为首选」（只有一家提供的模型不显示）。选择器展示、同步和计费都按首选那一行的名称与单价；调用先走首选供应商的全部地址，都失败才切到下一家，切换后仍按首选的单价计费（用户看到的是那个价）。排序依据统一为 `priority ASC, sort_order ASC, id ASC`，设为首选只改 priority，不影响模型在选择器里的位置 |
...
4. 无积分账户发送请求，网关返回 OpenAI 兼容的 HTTP 429 + `insufficient_quota`，LobeHub 会展示额度不足而不是通用 500。已有任务占满并发时返回 429；日限额/累计额度用完也返回 429。
