...
| 凭证安全 | 第三方密钥加密存库、后台只显示是否已设置、从不下发前端；上游若在报错里回显密钥，网关只把密钥替换为 `[REDACTED]`，其余原文照返。接入地址允许 http（不少中转站只有 http），但明文传输意味着密钥在网络上裸露，后台会在该地址旁标出；指向内网或本机的地址一律拒绝，避免持有模型权限的运营借「拉取模型」探测内网 |
