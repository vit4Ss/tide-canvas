...
export default function AdminAiLogsPage() {
  ...
  const [banType, setBanType] = useState<"ip">("ip");
  ...
  }, [pageNum, operationType, success, taskId]);
  ...
  const handleManualBan = async () => {
    if (!banValue.trim()) { toast.error("请输入 IP 地址"); return; }
  ...
  const columns: ColumnsType<AiGenerationLogVO> = [
    ...
    { title: "用户", key: "user", render: (_, l) => <span style={{ fontSize: 12 }}>{l.userName || "-"}</span> },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* 详情 */}
      <Modal title="操作日志" open={!!detail} onCancel={() => setDetail(null)} footer={null} width={760}>
        {detail && (
          <>
            <Descriptions size="small" column={2} bordered items={[
              ...
              { key: "proj", label: "画布", children: detail.projectName || "-" },
      ...
      <Modal title="封禁管理" open={bansOpen} onCancel={() => setBansOpen(false)} footer={null} width={680}>
        <Space.Compact style={{ width: "100%", marginBottom: 16 }}>
          <Select style={{ width: 100 }} value={banType} onChange={setBanType} options={[{ value: "ip", label: "IP" }]} />
          <Input placeholder="IP 地址" value={banValue} onChange={(e) => setBanValue(e.target.value)} />
