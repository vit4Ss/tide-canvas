...
import { Table, Input, Button, Modal, Tag, Avatar, Space, Alert, Select, Popconfirm } from "antd";
...
export default function AdminAuthorsPage() {
  ...
  const [grantUsers, setGrantUsers] = useState<AdminUserVO[]>([]);
  ...
  const searchGrantUsers = async (value: string) => {
    setGrantSearching(true);
    try {
      const res = await adminApi.users.list({ pageNum: 1, pageSize: 20, keyword: value || undefined });
      if (res.success) setGrantUsers(res.data.records);
    } finally {
      ...
      title: "作者", key: "user", render: (_, a) => (
        <Space>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "#bfbfbf" }}>@{a.username}</div>
          </div>
        </Space>
      ),
    },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <AdminPageHead
        title="作者管理"
        desc="管理签约作者的权限"
        extra={can("author:manage") && <Button type="primary" icon={<UserAddOutlined />} onClick={() => { setGrantOpen(true); void searchGrantUsers(""); }}>授权作者</Button>}
      />
      ...
      <Modal title="授权作者" open={grantOpen} onCancel={() => setGrantOpen(false)} onOk={handleGrant} confirmLoading={granting} okText="确认授权" cancelText="取消" okButtonProps={{ disabled: !grantUserId }}>
        <div style={{ paddingTop: 8 }}>
          <div style={{ marginBottom: 6 }}>用户</div>
          <Select
            showSearch
            filterOption={false}
            ...
            value={grantUserId}
            onSearch={searchGrantUsers}
            onChange={setGrantUserId}
            options={grantUsers.map((u) => ({ value: u.id, label: `${u.nickname || u.username}（${u.username}）` }))}
          />
        </div>
      </Modal>
    </div>
