...
import { Image, Popconfirm } from "@douyinfe/semi-ui";
import { Image as AntImage } from "antd";
import { aiApi, fileApi, pointsApi } from "@/lib/api";
import { formatDate } from "@/lib/utils";
import { useCanvasStore } from "@/stores/use-canvas-store";
import { AiTaskStatus, type AiGenerationLogVO, type AiTaskVO } from "@/types/ai";
import { POINTS_TYPE_NAMES, type PointsBalanceVO, type PointsTransactionVO } from "@/types/points";
...
export function CanvasHistoryPanel({ open, onClose, onAddResource }: Props) {
  ...
  const loadPoints = useCallback(async () => {
    ...
    try {
      const [balanceRes, txRes] = await Promise.all([
        pointsApi.balance(),
        pointsApi.transactions({ pageNum: pointsPage, pageSize: POINTS_PAGE_SIZE }),
      ]);
  ...
  const markDeleting = (id: number, deleting: boolean) => {
  ...
  const handleDeleteResource = async (item: AiGenerationLogVO) => {
    if (deletingIds.has(item.id)) return;
    markDeleting(item.id, true);
    try {
      const res = await aiApi.deleteLog(item.id);
      if (res.success) {
        setLogs((current) => current.filter((log) => log.id !== item.id));
        toast.success("已删除资源记录");
      } else {
                        ...
                        <article key={item.id} className="group overflow-hidden rounded-2xl border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-950">
                          ...
                          <div className="p-3">
                            ...
                            <div className="mt-3 flex items-center gap-2">
                              ...
                              <Popconfirm
                                title="确认删除这条资源记录？"
                                content="删除后不会再出现在当前画布资源历史中。"
                                okText="删除"
                                cancelText="取消"
                                okType="danger"
                                position="topRight"
                                onConfirm={() => void handleDeleteResource(item)}
                              >
                                <button
                                  type="button"
                                  disabled={deleting}
                                  className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-red-100 text-red-500 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-red-900/40 dark:hover:bg-red-950/30"
                                  title="删除"
                                >
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center justify-between gap-2">
                              <p className="truncate text-sm font-medium text-neutral-900 dark:text-neutral-100">{tx.typeName || POINTS_TYPE_NAMES[tx.type] || "积分变动"}</p>
